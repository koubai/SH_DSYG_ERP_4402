...
<script type="text/javascript">
	...
	function addProduct() {
		...
		//颜色
		var color = inputs[6].value;
		//包装
		var unit = inputs[15].value;
		//形式
		var packaging = inputs[10].value;
		
		getOpener().document.getElementById("strProductid").value = id;
		getOpener().document.getElementById("productname").value = tradename;
		getOpener().document.getElementById("productdetail").value = "品名：" + tradename + " 规格：" + typeno + " 颜色：" + color + " 包装：" + unit + " 形式：" + packaging;
		window.close();
	}
	
	function queryList() {
		$("#startIndex").attr("value", "0");
		document.mainform.action = '../product/queryDetailProductSelectPage.action';
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '../product/turnDetailProductSelectPage.action';
		document.mainform.submit();
	}
					...
					<s:iterator id="productList" value="productList" status="st1">
						...
						<s:else>
							...
							<td style="display: none;">
								...
								<input type="hidden" value="<s:if test='%{productList[#st1.index].packaging == "0"}'>整箱</s:if><s:elseif test='%{productList[#st1.index].packaging == "1"}'>乱尺</s:elseif><s:else>乱尺</s:else>"/>
				...
				<tr>
					<td>
						<div class="btn">
							<div class="box1_left"></div>
							<div class="box1_center">
								<input type="button" class="input80" value="确定" onclick="addProduct();"/>
							</div>
							<div class="box1_right"></div>
						</div>
						<div class="btn">
